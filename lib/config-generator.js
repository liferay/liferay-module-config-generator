...
ConfigGeneraror.prototype = {
    ...
    _generateModuleName: function(file) {
        ...
        if (fs.existsSync(instance._options.moduleConfig)) {
            moduleConfig = require(instance._options.moduleConfig);
        }

        fileName = path.join(path.dirname(file), fileName);

        var moduleName = path.join(moduleConfig.name, fileName.substring(instance._options.moduleRoot.length));
