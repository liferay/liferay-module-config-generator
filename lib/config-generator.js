...
ConfigGeneraror.prototype = {
    ...
    _getConfig: function(file, ast) {
        ...
        return new Promise(function(resolve, reject) {
            ...
            var isRequire = false;
            jsstana.traverse(ast, function(node) {
                var match = jsstana.match('(or (call define ?) (call define ? ?) (call define ? ? ?) (call require ?) (call require ? ?) (call require ? ? ?))', node);

                if (match) {
                    var checkNamespace = self._options.namespace && !self._options.skipFileOverride;
                    if (checkNamespace) {
                        var name = node.callee.name;
                        isRequire = name === 'require';
