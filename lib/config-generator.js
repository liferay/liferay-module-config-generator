...
ConfigGeneraror.prototype = {
    ...
    _generateModuleName: function(file) {
        ...
        if (instance._options.moduleConfig) {
            var fileModuleConfig = path.resolve(instance._options.moduleConfig);
        ...
        moduleName = path.join(moduleName, fileName.substring(instance._options.moduleRoot.length));
                ...
                if (match) {
                    ...
                    if (node.arguments.length === 1) {
                        ...
                        if (node.arguments[0].type === 'ArrayExpression') {
                            ...
                            node.arguments.splice(1, 0, builders.arrayExpression(dependencies));
                        }

                        // Save the file back
                        if (!instance._options.skipFileOverride) {
                            fs.writeFileSync(file, recast.prettyPrint(ast, {
                                wrapColumn: 1024
                            }).code);
                        }
                    } else {
    ...
    _onWalkerFile: function(root, fileStats, next) {
        ...
        if (minimatch(file, instance._options.filePattern, {
            dot: true
        })) {
