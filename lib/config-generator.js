...
ConfigGeneraror.prototype = {
    ...
    _getConfig: function(file, ast) {
        ...
        return new Promise(function(resolve, reject) {
            ...
            var isRequire = false;
            jsstana.traverse(ast, function(node) {
                var match = jsstana.match('(or (call define ?) (call define ? ?) (call define ? ? ?) (call ?.define ?) (call ?.define ? ?) (call ?.define ? ? ?) (call require ?) (call require ? ?) (call require ? ? ?) (call ?.require ?) (call ?.require ? ?) (call ?.require ? ? ?))', node);
                ...
                if (match) {
                    if (self._options.namespace)  {
                        ...
                        isRequire = (name && name === 'require') || (property && property.name === 'require');

                        if (name && name === 'define' || name === 'require') {
                            node.callee.name = self._options.namespace + '.' + name;
                        } else if (object.name !== self._options.namespace) {
                            object.name = self._options.namespace;
                        }
                    }

                    if (isRequire && !self._options.skipFileOverride) {
